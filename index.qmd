...
- *Principal Component 1:* seems to capture general intelligence (or as Spearman would called it the "g" factor) since all the subtests have positive loadings and most of them are strong. This suggests that PC1 might be measuring a general cognitive ability that affects performance across all the subtests.
...
#| tbl-cap: P-value for four proposed Factor Analysis model
...
tibble(
  ...
  TLI = c(fa_01$TLI, fa_02$TLI, fa_03$TLI, fa_04$TLI),
...
\lambda_2 = \pm \sqrt{6\left(\frac{7}{8}\right)}
\end{align}
$$
...
$$
\begin{align}
\hat{\lambda}_1 &= 8\left(\sqrt{6\left(\frac{7}{8}\right)}\right)^{-1}  &  \hat{\lambda}_2 &= \sqrt{6\left(\frac{7}{8}\right)} &  \hat{\lambda}_3 &= \frac{7}{8}\left(\sqrt{6\left(\frac{7}{8}\right)}\right)\\
\end{align}
$$
...
$$
\begin{align}
\hat{\psi}_1 &= 25- \left[ 8\left(\sqrt{6\left(\frac{7}{8}\right)}\right)^{-1}\right]^2  &  \hat{\psi}_2 &= 15- \left[ \sqrt{6\left(\frac{7}{8}\right)}\right]^2 &  \hat{\psi}_3 &= 10 - \left[ \frac{7}{8}\left(\sqrt{6\left(\frac{7}{8}\right)}\right)\right]^2\\
\end{align}
$$
